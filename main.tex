...
% Reason why process trees are a good thing to use instead of petri nets, BPMN etc...
...
\begin{table}[H]
\centering
\caption{An overview of the different periods of the development of the tool, along with an approximation in time.}
\label{table:planning}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|l|l|l|}
\hline
Objective & Description & Duration in weeks \\ \hline
\begin{tabular}[c]{@{}l@{}}Organizing a workflow logging data set, \\ which can be used as input for our tool.\end{tabular} & \begin{tabular}[c]{@{}l@{}}This phase exists of setting up and running a specific query over the Profit workflow \\ database, which results in a large .csv-file. Then, the result needs to be organized \\ per workflow id and sorted in the correst way, like explained earlier.\end{tabular} & 1 \\ \hline
Setting up a word2vec model. & \begin{tabular}[c]{@{}l@{}}This phase consists of training a word2vecmodel on the task- and action descriptions \\ that are used in our data set.\end{tabular} & 2 \\ \hline
Applying the Inductive Miner algorithm. & \begin{tabular}[c]{@{}l@{}}The algorithm is implemented in multiple plug-ins of the ProM framework. It would \\ be nice to use such an implementation. However, this requires a decent understanding\\ of the ProM framework etc. Nevertheless, I suspect that writing my own \\ implementation will be much more time-consuming.\end{tabular} & 4 \\ \hline
\begin{tabular}[c]{@{}l@{}}Returning occurrences of a given \\ workflow pattern.\end{tabular} & \begin{tabular}[c]{@{}l@{}}Lastly, we need to come up with a way to mine occurrences of a given workflow \\ pattern in a discovered workflow model (which is in the case of the Inductive \\ Miner a petri net).\end{tabular} & 1 \\ \hline
\end{tabular}%
}
\end{table}
